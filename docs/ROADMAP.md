...
- [x] Token saving: a deterministic per-request pipeline (RTK/Slimmer tool-output compression, optional Headroom proxy, terse/caveman output directives with ponytail stacked on top) that every chat request runs through before provider translation; each saver toggles independently, fails open, and is priced into a savings rollup on the Usage, Token Saving and Prometheus surfaces; a playground (`/playground`) whose Token Saver tab runs the real pipeline on a sample request and reports each step's measured effect, with per-run overrides that are never saved, and whose Chat tab streams a multi-turn completion through the same resolver, executor and pipeline and reports the answering tier, tokens, cost and prompt savings
...
- [x] Built-in provider presets (`/api/providers`): 55 one-click endpoints across
      three tiers (`api_key` / `free_tier` / `local`), each with a preset base URL,
      wire family and default headers, grouped into picker sections and shown as a
      `provider_id` badge on connections. Account-specific endpoints
      (`azure-openai`, `cloudflare`) ship a `<placeholder>` that
      `ConnectionRepository::validate_base_url` refuses to store. URLs mirror
      9Router's registry. Fixed here: the `anthropic` preset was missing its `/v1`
      segment, so the provider layer appended `/messages` to the bare host.
...
- [x] Command Code upstream (`command-code`): the Provider API with an automatic
