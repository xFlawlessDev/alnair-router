...
describe('ProviderIcon', () => {
  it('inlines the bundled glyph for a known provider', async () => {
    ...
    expect(container.querySelector('.bg-muted'), 'no monogram fallback').toBeNull();
  ...
  it('uses the wire family when a connection has no preset', async () => {
    const openai = mount({ id: null, type: 'openai-compatible', label: 'openai-compatible' });
    await nextTick();
    expect(openai.container.querySelector('svg'), 'openai-compatible').toBeTruthy();
    openai.unmount();

    const anthropic = mount({ id: null, type: 'anthropic-native', label: 'anthropic-native' });
    await nextTick();
    expect(anthropic.container.querySelector('svg'), 'anthropic-native').toBeTruthy();
    anthropic.unmount();

    // Command Code has no glyph yet, so it falls through to the monogram.
    const commandCode = mount({ id: null, type: 'command-code', label: 'Command Code' });
    await nextTick();
