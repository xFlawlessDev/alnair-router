...
async function run(): Promise<void> {
  ...
  try {
    ...
    toast.error(caught instanceof ApiError ? caught.message : 'Chat test failed');
  } finally {
    running.value = false;
  }
}
  ...
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogScrollContent class="sm:max-w-2xl">
      <DialogHeader>
        ...
        <DialogDescription>
          Runs one real non-streaming completion through the resolver and executor, so it exercises
          the same path as a client request.
        </DialogDescription>
      </DialogHeader>
      ...
      <div class="grid gap-4">
        ...
        <div v-if="result" class="grid gap-3 rounded-md border p-4">
          ...
          >
            <div><dt class="inline font-medium">Source </dt><dd class="inline">{{ result.source }}</dd></div>
            <div><dt class="inline font-medium">Model </dt><dd class="inline">{{ result.model }}</dd></div>
            <div><dt class="inline font-medium">Provider </dt><dd class="inline">{{ result.provider_type }}</dd></div>
            <div><dt class="inline font-medium">Tiers </dt><dd class="inline">{{ result.attempts }}</dd></div>
            <div>
            ...
            <div><dt class="inline font-medium">Cost </dt><dd class="inline">{{ formatCost(result.cost_usd ?? 0) }}</dd></div>
            <div><dt class="inline font-medium">Latency </dt><dd class="inline">{{ formatLatency(result.latency_ms ?? 0) }}</dd></div>
            <div v-if="result.finish_reason">
              <dt class="inline font-medium">Finish </dt><dd class="inline">{{ result.finish_reason }}</dd>
            </div>
          </dl>
        </div>
      </div>
