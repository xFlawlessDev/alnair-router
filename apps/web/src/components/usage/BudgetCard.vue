...
const hasCaps = computed(() => configured.value.length > 0 || configuredTokens.value.length > 0);
...
<template>
  <Card>
    <CardHeader>
      ...
      <CardDescription>Your spend against configured budget limits.</CardDescription>
    </CardHeader>
    <CardContent class="grid gap-4">
      <template v-if="hasCaps">
        <div v-for="{ cap, amount } in configured" :key="cap.field" class="grid gap-1.5">
          <div class="flex items-baseline justify-between gap-2 text-sm">
            <span class="text-muted-foreground">{{ cap.label }}</span>
            <span
              :class="[
                'tabular-nums',
                spentFor(cap.field) >= amount ? 'font-medium text-destructive' : '',
              ]"
            >
        ...
        >
          <div class="flex items-baseline justify-between gap-2 text-sm">
            ...
            <span
              :class="[
                'tabular-nums',
                tokensSpentFor(cap.field) >= amount ? 'font-medium text-destructive' : '',
              ]"
            >
