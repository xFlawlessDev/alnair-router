...
const emit = defineEmits<{ create: []; edit: [plan: KeyPlan]; remove: [plan: KeyPlan] }>();
</script>
  ...
  <Card>
    <CardHeader class="flex flex-row items-start justify-between gap-4">
      <div class="grid gap-1">
        ...
        <CardDescription>Set the rules once, slap them on any key.</CardDescription>
      </div>
      <Button variant="outline" @click="emit('create')"><Plus /> New plan</Button>
    </CardHeader>

    <CardContent>
      <p v-if="!plans.length" class="text-sm text-muted-foreground">
        No plans yet. Bundle a model allowlist, a rate limit and a budget, then apply the plan to
        any key with one click.
      </p>
        ...
        >
          <div class="grid min-w-0 gap-1">
            ...
            <div class="flex flex-wrap gap-1 pt-1">
              ...
              <span v-else class="text-xs text-muted-foreground">All models</span>
            </div>
          </div>
