...
const budgetCount = computed(
  () =>
    [props.daily, props.weekly, props.monthly, props.lifetime].filter((value) =>
      value.trim(),
    ).length,
);
...
const summary = computed(() => {
  ...
  if (budgetCount.value) {
    parts.push(
      `${budgetCount.value} USD budget${budgetCount.value > 1 ? "s" : ""}`,
    );
  }
  if (tokenCount.value) {
    parts.push(
      `${tokenCount.value} token limit${tokenCount.value > 1 ? "s" : ""}`,
    );
  }
      ...
      >
        <span class="grid gap-0.5">
          <span class="text-sm font-medium"
            >Budgets, token limits &amp; expiry</span
          >
        ...
        <div class="grid gap-3 border-t p-3">
          <div>
            ...
            <p class="text-xs text-muted-foreground">
              Caps stack: a request is refused when any configured window is
              exhausted. Daily, weekly and monthly reset on the UTC calendar;
              lifetime never resets. Blank windows are uncapped.
            </p>
          </div>

          <div class="grid gap-3 sm:grid-cols-2">
            <div v-for="window in WINDOWS" :key="window.key" class="grid gap-2">
              <Label :for="`${idPrefix}-budget-${window.key}`">{{
                window.label
              }}</Label>
              <Input
          ...
          <div class="grid gap-3 border-t pt-3">
            <div>
              ...
              <p class="text-xs text-muted-foreground">
                Enforced with the same mode as the USD budgets; blank windows
                are uncapped.
              </p>
            </div>
            <div class="grid gap-3 sm:grid-cols-2">
              <div
                v-for="window in TOKEN_WINDOWS"
                :key="window.key"
                class="grid gap-2"
              >
                <Label :for="`${idPrefix}-tokens-${window.key}`">{{
                  window.label
                }}</Label>
                <Input
          ...
          <div class="grid gap-3 border-t pt-3 sm:grid-cols-2">
            <div class="grid gap-2">
              ...
              <Select
                ...
                @update:model-value="
                  emit('update:budget-mode', $event as BudgetMode)
                "
              >
          ...
          <p class="text-xs text-muted-foreground">
            <template v-if="scope === 'plan'">
              Keys attached to an expired plan are rejected until the plan is
              extended.
            </template>
            <template v-else>
