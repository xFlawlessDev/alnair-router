...
const modelForms = [
  {
    ...
    kind: "Alias",
    detail:
      "The prefix oa routes to its connection and asks the upstream for gpt-4o-mini.",
  },
  {
    ref: "oa",
    kind: "Alias",
...
const openAiEndpoints = [
  ...
  [
    ...
    "Proxied, multipart pass-through.",
  ],
  [
    "POST /v1/videos/generations · GET /v1/videos/{id}",
    "Proxied, async polling included.",
  ],
  [
    "POST /v1/search · POST /v1/web/fetch",
    "Proxied search and SSRF-guarded server-side fetch.",
  ],
];
...
const anthropicEndpoints = [
  [
    "POST /v1/messages",
    "Translates to the shared executor with proper event ordering.",
  ],
  [
    "POST /v1/messages/count_tokens",
    "Heuristic estimate, no tokenizer dependency.",
  ],
];
...
const responseHeaders = [
  ...
  [
    "x-router-provider",
    "Wire protocol: openai-compatible or anthropic-native.",
  ],
...
const errors = [
  [
    ...
    "Missing, unknown, disabled or expired router key.",
  ],
  [
    "403",
    "permission_error",
    "Model outside the key's allowlist, or the plan has expired.",
  ],
  ["404", "not_found_error", "The model reference resolves to nothing."],
  [
    "402",
    "insufficient_quota",
    "A budget or token window (daily/weekly/monthly/lifetime) is exhausted in block mode.",
  ],
  [
    "429",
    "rate_limit_error",
    "Token bucket or concurrency cap; honors Retry-After.",
  ],
  [
    "502",
    "upstream_error",
    "Every tier failed; the last upstream error is reported.",
  ],
];
</script>
  ...
  <div class="flex max-w-4xl flex-col gap-6">
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          The default bind is <code>http://127.0.0.1:7878</code>, so the
          OpenAI-compatible base URL is <code>http://127.0.0.1:7878/v1</code>.
          Mint a key on the API Keys page and send it as a bearer token; when
          <code>server.require_api_key = false</code> the header can be omitted.
        </CardDescription>
      </CardHeader>
...
curl http://127.0.0.1:7878/v1/chat/completions \
  ...
  -d '{"model":"free-forever","messages":[{"role":"user","content":"hi"}]}'</pre>
        <p>
          SDKs work unchanged: set <code>base_url</code> to the router and use
          the router key as the API key.
        </p>
        <pre class="overflow-x-auto rounded-md bg-muted p-3 text-xs">
...
stream = client.chat.completions.create(
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          The <code>model</code> field resolves against aliases, combos and the
          default connection.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <ul class="grid gap-2">
          ...
          >
            <code class="w-32 shrink-0 text-xs font-medium">{{
              form.ref
            }}</code>
            <span
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          The router speaks both wire protocols, so existing clients keep
          working.
        </CardDescription>
      </CardHeader>
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription
          >What happens after the router resolves the model
          reference.</CardDescription
        >
      </CardHeader>
      <CardContent class="grid gap-3 text-sm">
        <p>
          A combo walks its tiers until one produces a first chunk; failover
          only happens before the first byte, so an error after content has
          started surfaces to the client instead of silently switching
          upstreams.
        </p>
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Failures return an OpenAI-style envelope:
          <code>{ "error": { message, type, code } }</code>.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <ul class="grid gap-2">
          ...
          >
            ...
            <code class="w-40 shrink-0 text-xs text-muted-foreground">{{
              type
            }}</code>
            <span class="text-xs text-muted-foreground">{{ description }}</span>
          </li>
