...
const events = computed<ActivityEvent[]>(() => {
  ...
  return level.value === 'all' ? all : all.filter((event) => event.level === level.value);
});
...
async function refresh(): Promise<void> {
  try {
    ...
    error.value = caught instanceof ApiError ? caught.message : 'Failed to load activity';
  }
  ...
  <div class="flex min-h-0 flex-col gap-6 md:h-[calc(100svh-9rem)]">
    ...
    >
      <template #actions>
        ...
        <Select v-model="level">
          ...
          <SelectContent>
            <SelectItem v-for="item in LEVELS" :key="item.value" :value="item.value">
              {{ item.label }}
            </SelectItem>
          </SelectContent>
        </Select>
    ...
    <Card class="flex min-h-0 flex-1 flex-col overflow-hidden">
      <CardContent class="flex min-h-0 flex-1 flex-col gap-0 p-0">
        ...
        >
          <p v-if="!events.length" class="text-zinc-500">No events match this filter.</p>
          <div
          ...
          >
            ...
            <span class="w-10 shrink-0 uppercase" :class="levelClass(event.level)">
              {{ event.level }}
            </span>
            <span class="w-36 shrink-0 truncate text-zinc-400" :title="event.kind">
              {{ event.kind }}
            </span>
            <span class="w-28 shrink-0 truncate text-zinc-500" :title="event.connection ?? ''">
              {{ event.connection ?? '—' }}
            </span>
            <span class="min-w-0 flex-1 break-words">{{ event.message }}</span>
            <span v-if="event.latency_ms != null" class="shrink-0 text-zinc-500">
              {{ event.latency_ms }}ms
            </span>
            <span
              v-if="event.status"
              class="w-8 shrink-0 text-right"
              :class="event.status >= 400 ? 'text-amber-400' : 'text-emerald-400'"
            >
              {{ event.status }}
            </span>
          </div>
