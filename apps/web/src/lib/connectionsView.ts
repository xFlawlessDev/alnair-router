...
export const DEFAULT_FILTERS: ConnectionFilters = {
  search: "",
  type: "all",
  status: "all",
};
...
export function filterConnections(
  ...
  return connections.filter((connection) => {
    if (filters.type !== "all" && connection.provider_type !== filters.type)
      return false;
    if (
      filters.status !== "all" &&
      isEnabled(connection.enabled) !== (filters.status === "enabled")
    ) {
      return false;
    }
    if (!term) return true;
...
export function groupConnections(
  ...
  for (const connection of connections) {
    const key =
      group === "provider"
        ? (connection.provider_id ?? CUSTOM_GROUP_KEY)
        : connection.provider_type;
    const label =
      group === "provider"
...
export function loadViewPreferences(): {
  ...
  try {
    ...
    return {
      view: VIEWS.includes(view as ConnectionsView)
        ? (view as ConnectionsView)
        : fallback.view,
      group: GROUPS.includes(group as ConnectionsGroup)
        ? (group as ConnectionsGroup)
        : fallback.group,
    };
  } catch {
    return fallback;
