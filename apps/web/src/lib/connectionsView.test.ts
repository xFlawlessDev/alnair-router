...
const LABELS: Record<string, string> = { openai: 'OpenAI', anthropic: 'Anthropic', groq: 'Groq' };

/** Mirrors the page's `providerLabel` for preset-backed connections. */
const labelOf = (row: Connection): string => (row.provider_id ? (LABELS[row.provider_id] ?? row.provider_id) : '');

const openai = connection({ id: 'a', name: 'openai-main', provider_id: 'openai' });
...
const filters = (overrides: Partial<ConnectionFilters> = {}): ConnectionFilters => ({
  ...DEFAULT_FILTERS,
  ...overrides,
});
...
describe('connection filters', () => {
  ...
  it('matches the search term against name, url, type and provider label', () => {
    expect(filterConnections(all, filters({ search: 'claude' }), labelOf)).toEqual([anthropic]);
    expect(filterConnections(all, filters({ search: 'anthropic.com' }), labelOf)).toEqual([anthropic]);
    expect(filterConnections(all, filters({ search: 'command-code' }), labelOf)).toEqual([custom]);
    expect(filterConnections(all, filters({ search: 'gr' }), labelOf)).toEqual([]);
  });

  it('is case-insensitive and ignores surrounding whitespace', () => {
    expect(filterConnections(all, filters({ search: '  OPENAI ' }), labelOf)).toEqual([openai]);
  });

  it('filters by provider type and status', () => {
    expect(filterConnections(all, filters({ type: 'anthropic-native' }), labelOf)).toEqual([anthropic]);
    expect(filterConnections(all, filters({ type: 'command-code' }), labelOf)).toEqual([custom]);
    expect(filterConnections(all, filters({ status: 'enabled' }), labelOf)).toEqual([
      openai,
      anthropic,
    ]);
    expect(filterConnections(all, filters({ status: 'disabled' }), labelOf)).toEqual([custom]);
  });
...
describe('connection grouping', () => {
  ...
  it('keeps one unlabelled group when flat', () => {
    expect(groupConnections(all, 'none', labelOf)).toEqual([{ key: 'all', label: '', items: all }]);
  });

  it('groups by provider and sorts custom endpoints last', () => {
    const groups = groupConnections(all, 'provider', labelOf);

    expect(groups.map((group) => group.label)).toEqual(['Anthropic', 'OpenAI', 'Custom endpoints']);
