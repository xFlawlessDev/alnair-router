...
function alias(id: string, prefix: string, model: string | null = null, enabled = 1): Alias {
  return {
    id,
    prefix,
...
function combo(id: string, name: string, tiers: number, enabled = 1): ComboWithEntries {
  return {
    combo: {
      id,
      name,
...
describe('buildSuggestions', () => {
  it('keeps enabled aliases and combos, sorted and annotated', () => {
    const suggestions = buildSuggestions({
      aliases: [alias('a2', 'oa', 'gpt-4o-mini'), alias('a1', 'kr'), alias('a3', 'off', null, 0)],
      combos: [combo('c2', 'smart', 2), combo('c1', 'free', 0), combo('c3', 'off', 1, 0)],
    });

    expect(suggestions.aliases.map((item) => item.pattern)).toEqual(['kr', 'oa']);
    expect(suggestions.aliases[1]).toMatchObject({ id: 'alias:a2', detail: 'gpt-4o-mini' });
    expect(suggestions.combos.map((item) => item.pattern)).toEqual(['free', 'smart']);
